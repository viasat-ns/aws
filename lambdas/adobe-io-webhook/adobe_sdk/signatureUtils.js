...
const helpers = require('@adobe/aio-lib-events/src/helpers.js')
...
//    pubKey2Pem = await fetchPubKeyFromCacheOrApi(pubKeyUrl2, state)
    ...
    pubKey1Pem = await fetchPublicKeyFromCloudFront(pubKeyUrl1)
    logger.info('publicKey: %s', pubKey1Pem)

    pubKey2Pem = await fetchPublicKeyFromCloudFront(pubKeyUrl2)
    logger.info('publicKey: %s', pubKey1Pem)
...
//  return publicKey.toString()
  ...
  logger.info(`publicKeyFileName: ${publicKeyFileName}`)
  return await fetchKeyFromApiAndPutInCache(pubKeyUrl, state, publicKeyFileName)
}
...
async function verifySignature (digitalSignatures, rawSignedPayload, publicKeys, recipientClientId) {
  ...
  try {
    for (let i = 0; i < digitalSignatures.length; i++) {
      ...
      logger.info('verifying digital signature %s', publicKey)

      result = await cryptoVerify(digitalSignatures[i], publicKey, rawSignedPayload)

      /* istanbul ignore else */
      if (result) {
        logger.info('found valid digital signature')
        return true
      }
    }
  } catch (error) {
